...
<template>
	<div>
		...
		<template v-else>
			...
			<el-table v-loading="policyStore.loading" element-loading-text="Loading..." :data="displayItems"
		...
		<!-- Create / Edit Dialog -->
		<el-dialog v-model="dialogVisible" :title="isEditing ? 'Edit Policy' : 'Create Policy'" width="900"
			destroy-on-close>
					...
					<el-card shadow="never">
						...
						<el-form-item label="Effect" required>
							<el-select v-model="rule.effect" placeholder="Select effect" style="width: 200px">
								<el-option v-for="opt in effectOptions" :key="opt.value" :label="opt.label" :value="opt.value" />
							</el-select>
						</el-form-item>

						<!-- Actions -->
						<el-form-item label="Actions" required>
							<el-select v-model="rule.actions" multiple filterable placeholder="Select actions" style="width: 100%">
						...
						<div v-for="(subject, sIdx) in rule.subjects" :key="sIdx" style="margin-bottom: 12px">
							<el-card shadow="never" style="background-color: #fafafa">
								...
								<el-form-item label="Users">
									<el-input type="textarea" :rows="2" :model-value="arrToText(subject.users)"
										@update:model-value="subject.users = textToArr($event)" placeholder="One user per line" />
								</el-form-item>
								<el-form-item label="Groups">
									<el-input type="textarea" :rows="2" :model-value="arrToText(subject.groups)"
										@update:model-value="subject.groups = textToArr($event)" placeholder="One group per line" />
								</el-form-item>
								<el-form-item label="Service Accounts">
									<el-input type="textarea" :rows="2" :model-value="arrToText(subject.serviceAccounts)"
								...
								<div v-for="(claim, cIdx) in subject.claims" :key="cIdx"
						...
						<div v-for="(cluster, cIdx) in rule.clusters" :key="cIdx" style="margin-bottom: 12px">
							<el-card shadow="never" style="background-color: #fafafa">
								...
								<el-form-item label="Names">
									<el-input type="textarea" :rows="2" :model-value="arrToText(cluster.names)"
										@update:model-value="cluster.names = textToArr($event)" placeholder="One cluster name per line" />
								</el-form-item>
								<el-form-item label="Labels">
						...
						<div v-for="(resource, resIdx) in rule.resources" :key="resIdx" style="margin-bottom: 12px">
							<el-card shadow="never" style="background-color: #fafafa">
								...
								<el-form-item label="Namespaces">
									<el-input type="textarea" :rows="2" :model-value="arrToText(resource.namespaces)"
										@update:model-value="resource.namespaces = textToArr($event)"
										placeholder="One namespace per line" />
								</el-form-item>
								<el-form-item label="Names">
									<el-input type="textarea" :rows="2" :model-value="arrToText(resource.names)"
										@update:model-value="resource.names = textToArr($event)" placeholder="One resource name per line" />
								</el-form-item>
						...
						<div v-for="(condition, condIdx) in rule.conditions" :key="condIdx"
							style="display: flex; gap: 8px; margin-bottom: 8px">
